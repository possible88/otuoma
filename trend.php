...
$trend_query = mysqli_query($conn, "SELECT * FROM trends WHERE title='$titles'");
$trend_row = mysqli_fetch_array($trend_query);
$trend_title = $trend_row['title'];
...
<div class="container" style="">
 ...
 $trend_body = explode(" ",$trend_title);

    
if(count($trend_body) == 3) 
    $trend_post = mysqli_query($conn, "SELECT * FROM posts WHERE (body LIKE '%$trend_body[0]%' AND body LIKE '%$trend_body[2]%') AND user_closed='no'");

else if(count($trend_body) == 2) 
    $trend_post = mysqli_query($conn, "SELECT * FROM posts WHERE (body LIKE '%$trend_body[0]%' OR body LIKE '%$trend_body[1]%') AND user_closed='no'");
else
    $trend_post = mysqli_query($conn, "SELECT * FROM posts WHERE (body LIKE '%$trend_body[0]%' OR body LIKE '%$trend_body[0]%') AND user_closed='no'"); 


        if(mysqli_num_rows($trend_post) == 0)
     echo "Not available " . $titles;
    else
            ...
            if($imagePath != "") {
                $imageDiv= "<div class='postedImage'>
                  ...
                  if($videoPath != "") {
                     $videoDiv= "<div class='postedImage'>
                                 ...
                                 <video src='$videoPath' title='video/mp4' controls width='100%' height='300px' style='margin-left: -35px;'  >
                     </div>";               
                   }  
                    else{
                        $videoDiv="";
               ...
               <div class='card-body'>
                   ...
                   <spna class='card-link'>
                       <h5 class='card-title'>$trend_body<br> $imageDiv$videoDiv</h5>
                   </span>
               </div>
